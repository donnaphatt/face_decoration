...
def detectFacialLandmarks(image, mode='image', display = True):
    '''
    ...
    face_mesh = face_mesh_images if mode == 'image' else face_mesh_videos
    ...
    if display:
        ...
        plt.subplot(122);plt.imshow(output_image);plt.title("Output");plt.axis('off');plt.show()
...
if __name__ == "__main__":
    ...
    '''
    Test by read image and process in this file.
    '''
    # img = cv2.imread("asset/kid.jpeg")
    # face_mesh_results = face_mesh_images.process(img[:,:,::-1])
    # _, face_mesh_results = detectFacialLandmarks(img, mode='image', display=False)
    # if face_mesh_results.multi_face_landmarks:
    #     output_image, _ = isOpen(img, face_mesh_results, 'MOUTH', threshold=15, display=False)
